    ...
    try {
      let filtered = dummyProperties.filter((property) => {
        ...
        if (
          ...
          return false;
        if (
          searchFilters.propertyType === "rent" &&
          !property.price.includes("/mo")
        )
          return false;
        ...
        if (searchFilters.maxPrice > 0 && numericPrice > searchFilters.maxPrice)
          return false;

        // Apply quick price filter if specified
        if (searchFilters.quickPrice) {
          if (searchFilters.propertyType === "buy") {
            if (searchFilters.quickPrice === "low" && numericPrice > 300000)
              return false;
            if (
              searchFilters.quickPrice === "medium" &&
              (numericPrice < 300000 || numericPrice > 600000)
            )
              return false;
            if (searchFilters.quickPrice === "high" && numericPrice < 600000)
              return false;
          } else {
            // Rent prices
            if (searchFilters.quickPrice === "low" && numericPrice > 1500)
              return false;
            if (
              searchFilters.quickPrice === "medium" &&
              (numericPrice < 1500 || numericPrice > 3000)
            )
              return false;
            if (searchFilters.quickPrice === "high" && numericPrice < 3000)
              return false;
          }
        }
